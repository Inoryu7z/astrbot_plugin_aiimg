...
*   修复 aiimg_generate 等工具返回图片时，LLM 同时收到插件中文消息和框架英文指令、语义冲突的问题
*   现在 LLM 只收到插件的单一中文消息："根据要求生成的图片已直接发送给用户，无需调用 send_message_to_user 再次发送。现在用户和你在现在同时看到了这张图片，请根据你看到的图片内容，以符合你人设的口吻生成一段回复。"
...
*   新增 `/补拍 @提供商ID` 命令：针对单个提供商立即补拍，只消耗该提供商的剩余额度，不影响同人格下其它提供商
*   使用场景：某提供商因故未在定时任务中自拍或额度未耗尽时，手动触发该提供商的补拍；提前触发后定时任务到点时因额度已用完会自动跳过
*   命令行为：找不到提供商/额度已耗尽/任务正在运行 时均提示并退出，不启动任务
*   删除旧的 `/补画` 命令（全量补画命令，已不够用）；定时任务（_cron_loop）保留不变
*   `/补画状态` 改名为 `/补拍状态`，效果不变，不保留旧名 alias
*   多人格同时配置同一提供商的情况按设计不应出现，若意外出现则只取第一个人格补拍（已写进代码注释）
...
*   新增 `/补拍debug` 命令：基于内存缓冲区（deque）实时记录补拍重要事件，输出当日摘要（触发人格数、设计批次数、超时次数、延迟重试次数、最终成功/失败），并列出带时间戳的事件流
*   内存缓冲区方案不依赖日志文件路径，Docker/本地环境通用
*   补拍日志「开始处理人格 X，总剩余额度 Y（1个提供商）」改为「（提供商: jier5, xxx）」显示具体提供商名
...
*   补拍第2轮「创意设计」超时阈值从 360s 调整为 600s
*   设计批次整体失败后不再直接跳过，改为等待 10 分钟后延迟重试，最多 2 次
*   新增 23:30 跨日保护截止线：预计重试开始时间已到/晚于 23:30 时终止重试，避免生图跨日导致额度计算错乱
...
*   补拍第3轮「审核师」系统提示词现支持按人格单独配置，与第2轮「创意设计」和第4轮「提示词构建」保持一致的配置模式
*   每个人格块（selfie_persona_1/2/3）新增 `reviewer_system_prompt` 字段，预填完整默认提示词，用户可在 WebUI 中直接编辑
*   用户可根据角色特征调整审核尺度：例如某个角色对发型约束更严格、某个角色希望审核更激进/保守等
*   配置留空时自动回退到插件内置默认提示词，不影响现有行为
*   字段使用 markdown 全屏编辑器，与其他系统提示词字段风格一致

---

### v1.9.0

**🚀 补拍链路重构：r0算法选风格 + r3服装审核环节 + 全局provider配置**

*   **r0 改为算法选风格**：原第1轮 LLM 选风格替换为算法实现 `_select_styles_by_algorithm`，策略为近期去重+加权随机。省出一次 LLM 调用，且避免 LLM 走训练数据高频路径导致的风格刻板化
*   **新增 r3 服装审核环节**：在 r2 设计师输出后、r4 翻译前，插入 `_llm_round3_review` 审核环节，由审核师对每套设计做美学审查（风格纯度/色彩和谐/材质对话/廓形比例/视觉焦点/单品必要性 6 维度），未通过的设计直接由审核师给出改进版，不回炉 r2
*   **重写 `_COSTUME_DESIGNER_SYSTEM_PROMPT`**：吸收自用提示词的设计哲学——设计语言三要素锚定（色彩哲学/廓形语言/材质情绪）、经典搭配优先、单品必要性原则、6 维设计自查；砍掉原 5 重差异化压力约束，改为"每个配对独立设计"
*   **新增 `_COSTUME_REVIEWER_SYSTEM_PROMPT`** 审核师常量：前置锚定步骤 + 6 维审查 + 决策原则（风格一致性优先/保留亮点/实质提升/宁缺毋滥）+ 硬约束一票否决
*   **流程编排重构**：r0（算法选风格）+ r1（LLM 选场景）并行 → r2（设计师）→ r3（审核师）→ r4（提示词翻译）。审核失败容错返回原设计，不阻断流程
*   **全局 provider 配置**：4 个轮次（r1场景/r2设计/r3审核/r4翻译）的 LLM 提供商全部改为全局级配置，新增 `daily_selfie_scene_provider_id` / `daily_selfie_designer_provider_id` / `daily_selfie_reviewer_provider_id` / `daily_selfie_prompt_engineer_provider_id` 4 个配置项，留空则回退到 `daily_selfie_chat_provider_id`。删除 persona 级 `costume_designer_provider_id` 配置项
*   **审核师改进版 merge**：`_apply_reviews` 改为 `{**design, **improved}` merge 策略，防止审核师只返回修改字段导致 scene/pose/appearance 字段丢失
*   **方法名统一**：`_llm_round2_scene` → `_llm_round1_scene`，`_llm_round3_design` → `_llm_round2_design`，方法名与协议位置一致
*   删除不再使用的 `_TASK_MODE_SYSTEM_PROMPT` / `_ROUND1_USER_PROMPT` 常量和 `_llm_round1` / `_get_costume_designer_provider_id` 方法
*   日志统一：r0/r1/r2/r3/r4 轮次命名一致
