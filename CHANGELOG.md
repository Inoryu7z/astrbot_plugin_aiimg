...
*   新增第三个自拍人格配置槽位 `selfie_persona_3`，结构与 `selfie_persona_2` 完全一致（参考人像、自拍链路、补画提供商、视频链路、发空间、创意设计等）
*   扩展所有人格遍历逻辑：`main.py` 与 `core/daily_selfie.py` 中所有 `for idx in [1, 2]` 循环升级为 `for idx in [1, 2, 3]`，覆盖自拍参考照查找、链路查找、视频链路查找、完整配置查找、自拍计数、补画启用扫描等全部路径
*   同步更新相关 docstring 与注释，反映三人格槽位
