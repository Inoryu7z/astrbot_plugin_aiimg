...
让 Bot 像真人一样每天自己发发自拍、养号经营人设。每张补拍图经过以下环节，每个环节的 LLM 提供商均可全局独立配置：

| 环节 | 职责 |
|------|------|
| 算法选风格 | 不调 LLM，按"近期去重 + 加权随机"从风格池选风格，避免 LLM 训练数据导致的刻板化 |
| 场景生成 | LLM 为每套搭配生成拍摄场景 |
| 创意设计 | LLM 设计师按色彩 / 廓形 / 材质三要素产出服装+外观+姿势+场景的结构化设计 |
| 服装审核 | LLM 审核师从 6 个维度审查，未通过直接给改进版 |
| 提示词翻译 | 把设计稿翻译成最终生图提示词 |

**人格级定制**：每个人格（最多 3 个）可独立配置风格池、三段系统提示词（设计师/审核师/提示词工程师）、自拍与视频服务商链路。

**服务商级额度**：补拍额度是服务商级共享资源（不是人格级）。每个服务商可独立设置每日额度和触发时间。失败自动重试，23:30 跨日保护截止。
...
**图片类**：OpenAI Images、OpenAI ImagesURL、Ark Seedream（字节 Seedream 系列专用）、OpenAI Chat 图、Gemini Images、Gitee Images、Gitee 异步改图、即梦（豆包）
