...
| 环节 | 职责 |
|------|------|
| 算法选风格 | 从风格池加权随机选风格（cosplay 权重可在设置中调整，默认 3 倍，其余风格为 1），避免 LLM 反复挑同一个风格 |
...
| `aiimg_asset_preview` | 自拍专用：精读部位素材参考图并返回描述与图片，素材池直接注入工具描述。需开启 `features.selfie.asset_ref_enabled`；使用素材后一般不再同时用衣橱参考图。注入素材前必须先预览该素材 |
...
**级联后端**：True Grok（图片或视频皆可，fallback_chain 内最多 8 个同类型后端，依次调用失败自动顺延；kind 自动推断，不支持图片与视频混用、不支持嵌套级联）
